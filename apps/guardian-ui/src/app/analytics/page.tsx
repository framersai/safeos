...
export default function AnalyticsPage() {
  ...
  const [requiresMonitoringServer, setRequiresMonitoringServer] = useState(false);

  const fetchAnalyticsData = useCallback(async () => {
    setLoading(true);
    setError(null);
    setNoDataAvailable(false);
    setRequiresMonitoringServer(false);

    if (backendStatus.api !== 'connected' || !backendConfig.apiUrl) {
      setAnalyticsData(null);
      setRequiresMonitoringServer(true);
      setLoading(false);
      return;
    }

    try {
      const response = await fetch(`${backendConfig.apiUrl}/api/analytics?range=${timeRange}`);
      if (response.ok) {
  ...
  }, [backendConfig.apiUrl, backendStatus.api, timeRange]);
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
          ...
          <p className="text-slate-400 max-w-md mb-6">
            {requiresMonitoringServer
              ? 'SafeOS is running fully local/offline on this device. Full analytics are generated by an optional monitoring server. Use History for the local timeline, or connect a monitoring server to enable analytics.'
              : noDataAvailable
          ...
          <div className="flex gap-4">
            <button
              onClick={() => (requiresMonitoringServer ? router.push('/history') : router.push('/monitor'))}
              className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-lg transition-colors font-medium"
            >
              {requiresMonitoringServer ? 'Open History (Local)' : 'Start Monitoring'}
            </button>
            {requiresMonitoringServer && (
              <button
                type="button"
